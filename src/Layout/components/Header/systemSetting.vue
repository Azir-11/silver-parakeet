    ...
    <n-drawer-content title="项目配置" closable>
      <section>
        ...
        <div class="flex flex-col">
          <div class="grid h-24 gap-2 grid-cols-9">
            <span
              v-for="(systemColor, index) in systemStore.appThemeList"
              :key="index"
              :style="{ 'background-color': systemColor }"
              class="w-8 h-8 cursor-pointer flex justify-center items-center"
              @click="togTheme(systemColor)"
            >
              <n-icon v-if="systemColor === systemStore.appTheme" size="18" color="#fff">
                <Checkmark />
              </n-icon>
            </span>
          </div>
