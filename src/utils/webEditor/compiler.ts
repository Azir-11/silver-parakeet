...
async function compileCSS(code, prep): Promise<string> {
  switch (prep) {
    case "Less":
      await compileLess(code)
        .then((res) => {
          code = res.css;
        })
        .catch((err) => {
...
async function compileVue2(code) {
  ...
  const styleCode = styleCodes
    .join("\n")
    .replace(
      /url\(\s*(?:(["'])((?:\\.|[^\n\\"'])+)\1|((?:\\.|[^\s,"'()\\])+))\s*\)/g,
      (_, quotes, relUrl1, relUrl2) => {
...
export { compileHTML, compileCSS, compileJS, compileVue2, compileVue3 };
